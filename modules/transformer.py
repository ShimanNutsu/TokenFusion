...
class PatchEmbedding(nn.Module):
    def __init__(self, in_channels, out_channels, patch_size, same=False):
        self.same = same
        super().__init__()
        if not same:
            self.embedder = nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=patch_size, stride=patch_size)
    def forward(self, x):
        if not self.same:
            x = self.embedder(x)
